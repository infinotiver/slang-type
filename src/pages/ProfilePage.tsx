  ...
  return (
    <div className="bg-background text-foreground flex flex-col font-mono">
      ...
      <main className="flex-1 py-4 sm:py-6 space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs text-foreground font-mono border border-secondary/40 bg-secondary/10 px-3 py-2 rounded-lg">
          {aiQuota ? (
            <>
              <span className="px-2 py-1 rounded-full">AI requests:</span>
              <span>
                {aiQuota.used} / {aiQuota.limit} today &bull; reset:{" "}
                {aiQuota.resetAt}
              </span>
            </>
          ) : (
            <span>unable to fetch AI usage stats</span>
          )}
        </div>
        {syncStatus === "syncing" ? (
